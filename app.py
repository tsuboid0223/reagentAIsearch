...
import json
from tqdm import tqdm # Streamlitではtqdm.notebookは使わない
import concurrent.futures

# === 3. AIエージェントと関連関数の定義 (Colab版から流用) ===
# これらの関数はUIに関係ないため、基本的にはそのまま使えます

def search_product_urls_with_brightdata(query, api_key):
    # Streamlitではprintの代わりにst.writeやst.infoを使うと画面に表示される
    ...
    try:
        ...
        for i in range(15): # タイムアウトを少し長めに
            time.sleep(2)
            ...
            if result_response.status_code == 200:
                try:
                    ...
                    st.error(f"レスポンスがJSON形式ではありませんでした。内容: {result_response.text}")
                    return []
...
def orchestrator_agent(product_info, gemini_api_key, brightdata_api_key, preferred_sites=[]):
    ...
    # Streamlitではtqdmの代わりにst.progressが使えるが、並列処理との組み合わせは複雑なためここでは省略
...
gemini_api_key = st.sidebar.text_input("Google Gemini APIキー", type="password")
brightdata_api_key = st.sidebar.text_input("Bright Data APIキー", type="password")

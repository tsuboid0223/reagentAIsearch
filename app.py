...
def search_product_urls_with_brightdata(query, api_key):
    """Bright Data APIを使い、Google検索結果のURLリストをJSON形式で取得する。"""
    st.info(f"【Bright Data】クエリ「{query}」でJSON形式の検索リクエストを送信...")
    ...
    payload = {
        'source': 'Google Search', 'q': query, 'country': 'JP',
        'parse': True, 'zone': 'serp_api1'
    }
    ...
    try:
        ...
        st.info(f"【Bright Data】リクエスト受付完了 (ID: {response_id})。結果を待機します...")

        result_url = f'https://api.brightdata.com/serp/get_result?response_id={response_id}'
        for i in range(15):
            st.write(f"  - ポーリング試行 {i+1}/15...")
            time.sleep(3) 
            ...
            if result_response.status_code == 200:
                try:
                    ...
                    if 'organic' in results and results['organic']:
                        ...
                        st.success(f"【Bright Data】合計{len(urls)}件のURLをJSONから抽出しました。")
                        return urls
                    else:
                        # ★★ ログ出力強化 ★★
                        st.warning("検索結果に有効なデータが含まれていませんでした。受信データを確認します。")
                        st.json(results) # 実際に受信したJSONデータを表示
                        return []
                except requests.exceptions.JSONDecodeError:
                    st.error(f"APIからの応答がJSON形式ではありませんでした。内容: {result_response.text}")
                    return []
            elif result_response.status_code != 202:
                # ★★ ログ出力強化 ★★
                st.error(f"結果取得エラー: {result_response.status_code}")
                st.error(f"エラーレスポンス: {result_response.text}") # エラーの本文を表示
                return []

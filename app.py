            ...
            if result_response.status_code == 200:
                try:
                    ...
                    if urls:
                        ...
                        st.warning("HTMLからURLを抽出できませんでした。")
                        return []
...
def orchestrator_agent(product_info, gemini_api_key, brightdata_api_key, preferred_sites=[]):
    """統括エージェント：検索と分析のモジュールを指揮し、最終的な結果を返す。"""
    ...
    base_queries = []
    base_queries.append(f"{manufacturer} {product_name}") # 元のクエリを追加
    ...
    all_urls = []
    for query in list(dict.fromkeys(search_queries)): # 重複する検索クエリを削除
